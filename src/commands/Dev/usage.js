...
async execute(interaction, client) {
  ...
  const otherPercentage = ((otherCommandsCount / totalUsage) * 100).toFixed(2);
  ...
  if (otherCommandsCount > 0) {
    let otherCommandsDetail = otherUsages.map(cmd => `${cmd.commandName}: ${cmd.count} times (${((cmd.count / totalUsage) * 100).toFixed(2)}%)`).join('\n');
    usageEmbed.addFields({
      name: `Other commands`,
